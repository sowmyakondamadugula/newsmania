...
// Function to fetch and display Sakshi news
/*const fetchSakshiNews = async () => {
    const newsContainer = document.getElementById('news-container');
    newsContainer.innerHTML = "<h3>SAKSHI NEWS</h3>";
    const url = `${proxyUrl}${encodeURIComponent("https://www.sakshi.com")}`;
    const response = await fetch(url);
    const data = await response.json();
    const parser = new DOMParser();
    const doc = parser.parseFromString(data.contents, 'text/html');
    const parasContainer = doc.querySelector('div.small-img-text-box.mt-0');
    if (parasContainer) {
        const headlines = parasContainer.querySelectorAll('h5.heading');
        const texts = parasContainer.querySelectorAll('p.text');

        for (let i = 0; i < headlines.length; i++) {
            const headlineText = headlines[i].innerText;
            const text = texts[i] ? texts[i].innerText : '';
            newsContainer.innerHTML += `<h3>${headlineText}</h3><p>${text}</p><br><br>`;
        }
    } else {
